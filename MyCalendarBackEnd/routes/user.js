...
  monthly_settings,
  sequelize,
  notes, // <--- WAŻNA POPRAWKA: Zaimportowanie modelu 'notes'
} = require("../db");
...
// ZMODYFIKOWANY ENDPOINT DLA Notes.jsx
router.get("/notatki/:userID/:year/:month", async (req, res) => {
  try {
    const { userID, year, month } = req.params;
    const monthFormatted = month.padStart(2, "0"); // Upewnij się, że miesiąc ma 2 cyfry

    const notesList = await notes.findAll({ // Zmiana na tabelę 'notes'
      where: {
        user_id: userID,
        [Op.and]: [
          sequelize.where(
...
          sequelize.where(
            sequelize.fn("strftime", "%m", sequelize.col("data")),
            monthFormatted // Użyj sformatowanego miesiąca
          ),
        ],
      },
      attributes: ["id", "data", "title", "description", "time"], // Pobierz potrzebne pola
      order: [["data", "ASC"], ["time", "ASC"]], // Sortuj wg daty, potem godziny
    });
...
// ZMODYFIKOWANY ENDPOINT - usunięcie logiki notatek
...
// ENDPOINT DLA PopUp.jsx
router.get("/notes/:userID/:date", async (req, res) => {
  const { userID, date } = req.params;
  try {
    const noteList = await notes.findAll({
      where: { user_id: userID, data: date },
      order: [["id", "ASC"]], // Zmieniono z 'time' na 'id' na wszelki wypadek, jeśli 'time' jest null
    });
    res.json(noteList);
  } catch (err) {
    console.error(err);
...
// ZMODYFIKOWANY ENDPOINT - dodanie 'time'
router.post("/add-note/:userID/:date", async (req, res) => {
  const { userID, date } = req.params;
  const { title, description, time } = req.body; // Dodano 'time'
...
// ... (reszta endpointów od /get-settings/ w dół - bez zmian) ...
